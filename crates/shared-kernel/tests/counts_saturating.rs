...
fn linecount_saturating_add_and_sub() {
    ...
    assert_eq!(
        LineCount::from(5).saturating_add_count(LineCount::from(usize::MAX)),
        max
    );
    let zeroed = LineCount::zero().saturating_sub(5);
    assert_eq!(zeroed, LineCount::zero());
    assert_eq!(
        LineCount::from(3).saturating_sub_count(LineCount::from(5)),
        LineCount::zero()
    );
}
...
fn charcount_saturating_add_and_sub() {
    ...
    assert_eq!(
        CharCount::from(2).saturating_sub_count(CharCount::from(3)),
        CharCount::ZERO
    );
}
...
fn wordcount_saturating_add_and_sub() {
    ...
    assert_eq!(
        WordCount::from(usize::MAX - 1).saturating_add_count(WordCount::from(5)),
        max
    );
    assert_eq!(
        WordCount::from(2).saturating_sub_count(WordCount::from(5)),
        WordCount::ZERO
    );
}
