        ...
        pub fn from_args(args: &crate::cli::Args) -> Result<Self> {
            ...
            Ok(Self {
                ...
                size_range: SizeRange::new(
                    args.min_size.map(|s| s.0),
                    args.max_size.map(|s| s.0),
                ),
    ...
    impl PathMatcher {
        ...
        fn matches_metadata(path: &Path, config: &Config) -> bool {
            ...
            let metadata = match std::fs::metadata(path) {
                Ok(m) => m,
                Err(_) => return true, // Skip metadata checks if unavailable
            };
...
mod compute {
    ...
    impl Aggregator {
        ...
        fn aggregate_by_ext(stats: &[FileStats]) -> (String, Vec<AggregationGroup>) {
            ...
            let groups = Self::map_to_sorted_groups(map);
            ("By Extension".to_string(), groups)
        }

        fn aggregate_by_dir(stats: &[FileStats], depth: usize) -> (String, Vec<AggregationGroup>) {
            let map = Self::build_aggregation_map(stats, |s| {
                crate::util::get_dir_key(&s.path, depth)
            });

            let groups = Self::map_to_sorted_groups(map);
            (format!("By Directory (depth={depth})"), groups)
        }
        ...
        fn aggregate_by_mtime(stats: &[FileStats], gran: crate::cli::Granularity) -> (String, Vec<AggregationGroup>) {
            ...
            (format!("By Mtime ({gran_label})"), groups)
        }
        ...
        fn map_to_sorted_groups(map: HashMap<String, (usize, usize, usize)>) -> Vec<AggregationGroup> {
            let mut groups: Vec<AggregationGroup> = map
                .into_iter()
                .map(|(key, (lines, chars, count))| {
                    AggregationGroup::new(key, lines, chars, count)
                })
                .collect();
...
mod compare {
    ...
    impl SnapshotComparison {
        ...
        fn format_summary_diff(&self) -> String {
            ...
            if let (Some(old_words), Some(new_words)) = (self.old.summary.words, self.new.summary.words) {
                output.push_str(&format!(
                    "Words: {} -> {} (Δ {})\n",
                    old_words,
                    new_words,
                    self.calculate_diff(new_words, old_words)
                ));
            }
