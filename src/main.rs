...
mod files {
    ...
    pub fn collect_entries(config: &AppConfig) -> anyhow::Result<Vec<Entry>> {
        if let Some(ref from0) = config.files_from0 {
            return read_files_from0(from0).map(|v| to_entries(v, config));
        }
        if let Some(ref from) = config.files_from {
            return read_files_from(from).map(|v| to_entries(v, config));
        }
        if config.use_git {
    ...
    fn to_entries(files: Vec<PathBuf>, config: &AppConfig) -> Vec<Entry> {
        files
            .into_iter()
            .filter_map(|p| meta_for(&p, config).map(|m| Entry { path: p, meta: m }))
            .collect()
    }
    ...
    pub fn collect_find_entries(config: &AppConfig) -> anyhow::Result<Vec<Entry>> {
        ...
        for root in &config.paths {
            ...
            for entry in walker {
                ...
                if let Some(meta) = meta_for(path, config) {
                    entries.push(Entry {
...
mod compute {
    ...
    fn measure(entry: &Entry, config: &AppConfig) -> Option<FileStats> {
        if !is_text_ok(entry, config) {
            return None;
        }
        if config.count_newlines_in_chars {
            measure_whole(&entry.path, entry, config)
        } else {
            measure_by_lines(&entry.path, entry, config)
        }
    }
    ...
    fn measure_by_lines(
        ...
        let st = FileStats {
            ...
            lines,
            chars,
            words: config.words.then_some(words),
            size: entry.meta.size,
            mtime: entry.meta.mtime,
            ext: entry.meta.ext.clone(),
            name: entry.meta.name.clone(),
        ...
        for key in by {
            ...
            match key {
                ...
                ByKey::Mtime(g) => {
                    ...
                    v.sort_by(|a, b| b.lines.cmp(&a.lines));
                    let gran = match g {
                        crate::config::Granularity::Day => "day",
                        crate::config::Granularity::Week => "week",
                        crate::config::Granularity::Month => "month",
                    };
                    results.push((format!("By Mtime ({})", gran), v));
                }
            }
        }
        results
...
mod output {
    ...
    fn output_jsonl(stats: &[t::Stats], config: &AppConfig, out: &mut impl Write) -> anyhow::Result<()> {
        ...
        let (tl, tc, tw) = totals(stats);
        let total = serde_json::json!({ "type": "total", "version": crate::VERSION, "lines": tl, "chars": tc, "words": if config.words { Some(tw) } else { None }, "files": stats.len() });
...
mod compare {
    ...
    pub fn run(old_path: &std::path::Path, new_path: &std::path::Path) -> Result<String> {
        ...
        out.push_str(&format!(
            ...
            old.summary.lines,
            new.summary.lines,
            (new.summary.lines as isize - old.summary.lines as isize)
        ));
        out.push_str(&format!(
            "Chars: {} -> {} (Δ {})\n",
            old.summary.chars,
        ...
        for nf in &new.files {
            if let Some(of) = om.get(nf.file.as_str()) {
                let dl = nf.lines as isize - of.lines as isize;
                let dc = nf.chars as isize - of.chars as isize;
                if dl != 0 || dc != 0 {
...
fn main() -> Result<()> {
    ...
    if let Some((old, newp)) = &config.compare {
        // 比較モード
